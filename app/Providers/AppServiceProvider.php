...
/**
 * @author Modified by Marty Zhang
 * @version 0.9.201809101136
 */
...
class AppServiceProvider extends ServiceProvider {
  ...
  public function boot() {
    ...
    Validator::extend('unique_multiple_fields', function ($attribute, $value, $parameters, $validator) {
      $result = FALSE;
      ...
      if (count($parameters) < 4 || empty($parameters[0]) || empty($parameters[1]) || empty($parameters[2]) || $parameters[3] == '') { // At lease two fields need to be provided.
        ...
        for ($i = 4; $i < count($parameters); $i += 2) {
    ...
    Validator::replacer('unique_multiple_fields', function ($message, $attribute, $rule, $parameters) {
